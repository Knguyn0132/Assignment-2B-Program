...
class SiteNetwork:
    ...
    def _process_road_connections(self, road, site_ids, max_distance, connections):
        ...
        for i in range(len(sites_on_road) - 1):
            ...
            if distance < max_distance:
                self._create_directional_connections(site1, site2, road, distance, connections)
    ...
    def _determine_road_orientation(self, sites_on_road):
        ...
        lat_variance = sum((lat - sum(lat_values)/len(lat_values))**2 for lat in lat_values)
        lng_variance = sum((lng - sum(lng_values)/len(lng_values))**2 for lng in lng_values)
        
        # If longitude varies more than latitude, it's likely an east-west road
        return lng_variance > lat_variance
    
    def _create_directional_connections(self, site1, site2, road, distance, connections):
        """
        ...
        self._try_create_connection(site1, site2, road, distance, approach_dir, connections)
        
        # Backward connection (site2 to site1)
        backward_dir = cardinal_direction
        self._try_create_connection(site2, site1, road, distance, backward_dir, connections)
    
    def _try_create_connection(self, from_site, to_site, road, distance, approach_dir, connections):
        """
